  ...
  <div class="flex flex-row items-center space-x-2">
    ...
    <button aria-label="Check for Updates"
      ...
      <Spinner v-if="updateChecking && !updateInfo"
               size="sm" />
      <DownloadIcon v-else-if="updateInfo"
                    :class="{ 'animate-pulse': !updateDownloaded, 'text-green-500': updateDownloaded }"></DownloadIcon>
...
const [discordConnected, setDiscordConnected] = refIpc(["discord.connected", "discord.disconnected"], {
	defaultValue: false,
	mapper: (data, name) => {
		return discordConnectionMap[name];
	},
	onTrigger(item, prev, { eventName }) {
